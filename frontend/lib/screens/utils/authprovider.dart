...
class AuthProviderClass extends ChangeNotifier {
  ...
  Future<void> signInWithGoogle() async {
    try {
      ...
      print("ACCESS TOKEN HERE $_accessToken");
  ...
  Future<String?> getValidToken() async {
   ...
   Future<String?> _refreshTokenSilently() async {
    try {
      ...
      if (googleUser == null) return null; // User is signed out

      final googleAuth = await googleUser.authentication;
      final freshToken = googleAuth.accessToken;
      await updateToken(freshToken, _refreshToken, 3600); // Assume 1h expiry
      print("FRESH TOKEN GUYS $freshToken");
      return freshToken;
    } catch (e) {
